...
#### How OOP enables Robustness of this program (can skip if already familiar with OOP)
This, as those familiar with OOP already know, enables the Airplane class to treat each of these components as a "black box." 
...
Again, as those familiar with OOP already know, this is an incredibly robust approach, since we are now able to change the implementation of major components, in a common-sense, safe, and simple way.
...
ever wanted to change from a trapezoidal wing assumption, all we need to do is change the implementation of the Wing Class (or create a derived class from Wing).
...
By programming using the aforementioned methodologies, particularly the classes I developed when creating this project, I am also able to re-use these classes for some of my other aero-related personal project (since these classes are, in many ways, black boxes).
